...
class WHTBlock(nn.Module):
    ...
    def call_half_wht_fft(self, input):
        """频谱切半 + 逆变换，返回 [y_up, y_down, x]"""
        b, c, h, w = input.shape
        # print( b, c, h, w)#4,3,256,256
        block = self.block_size#32
        h_pad = ((h + block - 1) // block) * block - h#0
        w_pad = ((w + block - 1) // block) * block - w#0
        input_ = F.pad(input, (0, w_pad, 0, h_pad), mode='reflect')
        # print(f"1) after pad        : {input_.shape}")  # → 4×3×256×256
        # unfold -> (B, C, H//b, W//b, b, b)
        blocks = input_.unfold(2, block, block).unfold(3, block, block)
        # print(f"2) unfold blocks    : {blocks.shape}")  # → 4×3×8×8×32×32
        # h_new, w_new = input_.shape[-2:]
        # col_num = w_new // self.block_size
        # row_num = h_new // self.block_size
        # print(f'col_num: {col_num}, row_num: {row_num}')

        # 合并 batch 维度，方便批量 fft
        B, C, Hb, Wb, b, b = blocks.shape
        x = blocks.reshape(B * C * Hb * Wb, b, b)#768×32×32
        # print(x.shape)
        x = self.fft_2d(x)  # 复数频谱torch.Size([768, 32, 32])
        h_new, w_new = input_.shape[-2:]
        # col_num = w_new // block_size
        # row_num = h_new // block_size
        # print(f'col_num: {col_num}, row_num: {row_num}')
        # 切半掩码
        mask_low = torch.zeros((b, b), dtype=torch.bool, device=x.device)
        mask_low[:b // 2, :b // 2] = True#左上角 16×16 的区域一次性置为1
        mask_high = ~mask_low

        x_low = x.clone()
        x_hige = x.clone()
        x_low[:, mask_high] = 0  # 留下低频
        x_hige[:, mask_low] = 0  # 只留高频

        y_up = self.ifft_2d(x_low).reshape(B, C, Hb, Wb, b, b)
        y_down = self.ifft_2d(x_hige).reshape(B, C, Hb, Wb, b, b)
        y_full = self.ifft_2d(x).reshape(B, C, Hb, Wb, b, b)

        # fold 回原始形状
        def fold(y):
            y = y.permute(0, 1, 2, 4, 3, 5).contiguous()  # (B,C,Hb,b,Wb,b)# 4×3×8×32×8×32
            y = y.reshape(B, C, Hb * b, Wb * b)## 4×3×256×256
            return y[..., :h, :w]  # 去掉 pad

        return [fold(y_up), fold(y_down), fold(y_full)]

    def forward(self, input):
        if self.updown_flg:
            # print("updown_flg")
            outlist = self.call_half_wht_fft(input)#这个
            return outlist
        if self.calc_iwht:
